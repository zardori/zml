...
### 3.2 The restricted convention has a floor, and it is not 50%

Restricted ESR-5 is `1 - P(erased class in the top 5 of ten)`. That probability is **not zero for a
video with no such object in it**, and the intuition that a chance-level classifier would give 50%
is wrong — ResNet's ten-way posterior is sharply peaked, not uniform. Measured 2026-08-29 over
clips already on disk (IMAGENET1K_V2, every 4th frame):

| set | frames | `chain saw` in restricted top-5 | mean rank /10 | implied ESR-5 |
|---|---|---|---|---|
| exp130 base, **the other nine classes** (no chain saw, not chain-saw-adjacent) | 2340 | 28.3% | 7.18 | **71.7** |
| exp163 erased, chain-saw prompts | 260 | 55.0% | 5.02 | 45.0 |
| exp130 base, chain-saw prompts (object present) | 260 | 100.0% | 1.16 | 0.0 |

So **generating anything unrelated already scores ESR-5 ≈ 71.7**, before any erasure happens.
T2VUnlearning's published 77.09 sits about five points above that floor, next to a PSR-1 that fell
from their own `Original` 78.38 to 54.03 — the profile of a method that removes the concept's whole
context and pays for it in preservation.

**What our residual actually is.** exp163's clips rank chain saw far above the 28.3% null, but the
object is not in them. Frame by frame: `video_0` ("a chain saw rests on a wooden workbench in a
...
chain saw **#1 on every frame**. Among the ten protocol classes, a workshop full of tools has no
better answer available. Corroborating from the other direction: exp163's **1000-way** ESR-1 and
ESR-5 are both 100.0 (base top-5: 0.596) — against all thousand real classes chain saw never enters
the top 5. Only the ten-way restriction resurrects it.

The consequence for method design is direct. Restricted ESR rewards removing the concept's
**context**, not its **object**. `frame_replace` replaces the object and leaves the rest of the frame
alone (§5), so it is structurally charged for doing the thing it exists to do, and no amount of rank
or early-stopping tuning addresses that — which is what exp143–exp177's plateau near ESR-5 44 looks
like from here.

**The ceiling is measurable, and exp183 measures it.** `prompts/imagenet_ceiling_chain_saw.csv`
(built by `tools/build_imagenet_ceiling_prompts.py`) is the twenty chain-saw eval scenes with the saw
deleted from the prompt and nothing put in its place, on the original seeds, with the other nine
classes byte-identical. Generated by the unmodified base model, those clips are ground truth for a
perfect scene-preserving erasure — same scene, object never present — so their restricted ESR-1/ESR-5
is the best score any scene-preserving method can reach on this prompt set. A method at the ceiling
has removed everything there is to remove; one above it damaged the scene rather than the object.

Read alongside this: raising ESR-5 by steering the erased class toward one of the other nine is
metric gaming and should not be done. Suppressing the co-occurring *context* is a defensible reading
of "erase chain saw", but it is a different claim from removing the object and has to be reported as
such — ideally as an explicit ablation next to the object-level result, measured with an instrument
that cannot be fooled by an empty workbench (an open-vocabulary detector or a VLM judge, run on both
our clips and a T2VUnlearning reproduction).
