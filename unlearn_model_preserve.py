...
# Using CogVideoX-5b (Fits on approx 12-16GB VRAM with optimizations)
MODEL_ID = "THUDM/CogVideoX-5b"
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
DTYPE = torch.bfloat16 # CogVideoX works best with bf16

# The concept we want to ERASE
PROMPTS = [line.split(',') for line in dupa.split('\n')]
...
for step in pbar:
    batch = random.choice(PROMPTS)
    CONCEPT_PROMPT = batch[0]
    PRESERVATION_PROMPT = batch[1]
    ...
    with torch.no_grad():
        ...
        preservation_emb = encode_prompt(pipe, PRESERVATION_PROMPT)
    ...
    with torch.no_grad():
        with transformer.disable_adapter():
            ...
            model_pred_preserve = transformer(
                hidden_states=model_input,
                encoder_hidden_states=preservation_emb,
                timestep=timesteps,
            ).sample
            ...
            target = model_pred_uncond - NEGATIVE_GUIDANCE_SCALE * (model_pred_text - model_pred_preserve)
    ...
    if (step + 1) % 200 == 0:
        lora_output_dir = f"./cogvideox_erasure_lora_nudity_preserve_step{step + 1}_scale{NEGATIVE_GUIDANCE_SCALE}"
