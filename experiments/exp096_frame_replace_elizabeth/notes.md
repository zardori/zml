...
(lowest id_sim, and the most degenerate-clip-affected — see exp090's notes.md), so Queen Elizabeth
II — next-highest id_sim, still non-US/female — is the corrected pilot identity.
...
**Specifically watch for identity-dependent results.** If Obama erased cleanly but Elizabeth doesn't
(or vice versa), that is itself the finding — worth understanding rather than averaging away, since
it would mean the split-prompt A/B/C recipe (or the erase regime) is not actually identity-agnostic
