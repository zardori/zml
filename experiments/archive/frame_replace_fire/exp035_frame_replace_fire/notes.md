...
takeaway: >
  First feasibility run of supervised v-prediction SFT toward the edited latent. Sits in the
  stabilized no-erasure regime (per exp043's diagnosis).
---
...
./submit_job.py athena experiments/archive/frame_replace_fire/exp034_frame_replace_precompute/config.yaml
# 2. point this experiment's metadata_file/latents_dir at that outputs_{timestamp} dir,
#    then submit training
./submit_job.py athena experiments/archive/frame_replace_fire/exp035_frame_replace_fire/config.yaml
```
