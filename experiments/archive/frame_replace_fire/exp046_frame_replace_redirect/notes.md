...
takeaway: >
  Denoising redirection (noise the original fire latent, regress toward the fireless target) —
  the breakthrough objective. Best erasure so far at step 500 (FDR 0.0, unrelated clip 0.33) but
  oscillating, and 2/5 concept videos near-grayscale.
---
...
2. Fill this config's `metadata_file` / `latents_dir` with the new precompute's
   ...
   reuse exp041 as-is.
3. **Train:** `./submit_job.py athena experiments/archive/frame_replace_fire/exp046_frame_replace_redirect/config.yaml`
