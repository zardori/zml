...
def main(config: Config) -> None:
    ...
    anchor_prompts = pd.read_csv(config.anchor_prompts_path)["prompt"].tolist()
    ...
    for step in pbar:
        ...
        with torch.no_grad():
            with transformer.disable_adapter():
                ...
                pred_anchor = transformer(
                    hidden_states=forget_model_input,
                    encoder_hidden_states=anchor_emb,
                    timestep=timesteps,
                ).sample

                esd_target = pred_uncond - config.negative_guidance_scale * (pred_concept - pred_anchor)
        ...
        if (step + 1) % config.save_interval == 0:
            ...
            evaluate(
                ...
                control_concept_prompts, control_related_prompts, control_unrelated_prompts, control_anchor_prompts,
            )
